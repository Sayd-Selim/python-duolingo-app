...
import BoldHighlight from "../../components/BoldHighlight";
...
function ExpressLesson2() {
  ...
  const steps = useMemo(
    () => [
      {
        type: "intro",
        title: "Как клиент передаёт данные серверу",
        content:
                        "Отлично! Теперь, когда наш почтовый офис работает, давайте научимся **принимать и обрабатывать данные от клиентов** — это сделает наш сервер по-настоящему полезным!\n\n" +
              "В нашем почтовом офисе есть три способа, как клиенты могут передать нам информацию:\n" +
              "• <CodeHighlight>URL Parameters</CodeHighlight> — данные прямо в адресе письма\n" +
              "• <CodeHighlight>Query Parameters</CodeHighlight> — данные в примечании на конверте\n" +
              "• <CodeHighlight>Request Body</CodeHighlight> — данные внутри самого письма\n\n" +
              "🎯 Наша цель: Научить наш почтовый офис читать и обрабатывать все эти типы данных!",
            motivation: "Умение принимать данные от клиентов — это основа создания форм, API и интерактивных веб-приложений!",
            metaphor: [
              "📮 Продолжаем работу нашего почтового офиса:",
              "Теперь наш офис не только отправляет ответы, но и принимает данные от клиентов.",
              "Клиенты могут передать информацию тремя способами:",
              "• ✉️ <CodeHighlight>URL Parameters</CodeHighlight> — как адрес получателя на конверте",
              "• 📝 <CodeHighlight>Query Parameters</CodeHighlight> — как примечание на конверте",
              "• 📄 <CodeHighlight>Request Body</CodeHighlight> — как содержимое самого письма",
              "Наш офис должен уметь читать все эти данные и правильно их обрабатывать!",
            ],
      },
      // {
      //   type: "theory",
...
//         ],
//         whatNext: [
//           "🚀 Урок 2: Парсинг тела запроса (Body Parsing)",
//           "В следующем уроке мы изучим:",
//           "- 📝 <CodeHighlight>req.body</CodeHighlight> — как читать содержимое писем (данные от клиента)",
//           "- 🔧 <CodeHighlight>body-parser</CodeHighlight> — middleware для обработки JSON и form-данных",
//           "- 📦 <CodeHighlight>POST</CodeHighlight> запросы — как принимать данные от форм",
//           "- 🛠️ <CodeHighlight>middleware</CodeHighlight> — промежуточные обработчики в Express.js",
//           "- 🎯 Создание полноценного API с возможностью приёма данных!",
//         ],
//       },
    ],
    []
  ...
  const parseJSXInText = (text) => {
    ...
    while (currentIndex < text.length) {
      ...
      const boldOpenTagIndex = text.indexOf("<BoldHighlight>", currentIndex);
      ...
      // Находим ближайший тег
      const tagIndices = [
        { index: codeOpenTagIndex, type: "code", openLength: 15, closeLength: 16 },
        { index: warningOpenTagIndex, type: "warning", openLength: 18, closeLength: 19 },
        { index: boldOpenTagIndex, type: "bold", openLength: 14, closeLength: 15 },
      ...
      const nearestTag = tagIndices.reduce((nearest, current) => (current.index < nearest.index ? current : nearest));

      openTagIndex = nearestTag.index;
      tagType = nearestTag.type;
      openTagLength = nearestTag.openLength;
      closeTagLength = nearestTag.closeLength;
      ...
      const closeTagName = tagType === "code" ? "</CodeHighlight>" : tagType === "warning" ? "</WarningHighlight>" : "</BoldHighlight>";
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-2">
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium text-gray-600">📚 Урок 2: Передача данных</span>
                ...
                <p key={index} className="mb-3">
                  {parseJSXInText(line).map((part, partIndex) =>
                    part.type === "code" ? (
                      ...
                      <BoldHighlight key={partIndex}>{part.content}</BoldHighlight>
                    ) : (
