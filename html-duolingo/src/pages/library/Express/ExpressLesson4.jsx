...
function ExpressLesson4() {
  ...
  const steps = useMemo(
    () => [
      {
        type: "code_files",
        title: "Охрана на входе — middleware 🛡️",
        content:
          "Наш отель работает отлично: сотрудники справляются, номера доступны, клиенты довольны. Но…\n\n" +
          "❗ Один клиент пришёл без документов.\n" +
          "😨 Другой попытался пробраться в люкс без брони.\n\n" +
          "Мы поняли: **нужна охрана на входе**, которая будет проверять каждого посетителя перед тем, как он попадёт к сотрудникам.\n\n" +
          "В Express такую проверку выполняет **middleware** — специальная функция, которая срабатывает **до маршрута**.\n\n" +
          "🔍 Она получает три аргумента:\n" +
          "— <CodeHighlight>req</CodeHighlight> — клиент, который зашёл (все его данные, поведение, заголовки и т.п.),\n" +
          "— <CodeHighlight>res</CodeHighlight> — рот охранника: он может сразу ответить, отказать или поприветствовать,\n" +
          "— <CodeHighlight>next()</CodeHighlight> — кнопка: нажал — и клиент пошёл дальше к сотрудникам (маршрутам).\n\n" +
          "Если <CodeHighlight>next()</CodeHighlight> не вызвать — клиент дальше не пройдёт.",
        codeFiles: {
          "server.js": `// 🛡️ Middleware: охранник, который проверяет, назвался ли клиент
      app.use((req, res, next) => {
        const name = req.headers['x-client-name']; // 🧾 Клиент показал документ
      
        if (!name) {
          return res.status(400).send('⛔ Вход запрещён без имени!'); // 👄 Рот охранника: отказ
        }
      
        console.log(\`✅ Клиент \${name} прошёл проверку\`);
        next(); // 🚪 Охранник нажал кнопку — проходи дальше
      });
      
      // 📍 Основной маршрут
      app.get('/', (req, res) => {
        res.send('Добро пожаловать в отель!');
      });`,
          "client.js": `// 🚀 Клиент отправляет запрос с заголовками
import axios from 'axios';

// ✅ Успешный запрос с именем в заголовке
const successfulRequest = async () => {
  try {
    ...
    // Выведет: "Добро пожаловать в отель!"
...
// ❌ Запрос без имени в заголовке
const failedRequest = async () => {
  try {
    const response = await axios.get('http://localhost:3000/');
    // Без заголовка x-client-name
    
    console.log('Ответ сервера:', response.data);
    
  } catch (error) {
    console.log('Ошибка доступа:', error.response?.data);
    // Выведет: "⛔ Вход запрещён без имени!"
  }
};
...
failedRequest();`
        ...
        metaphor: [
          "🛡️ Метафора:",
          "Клиент (req) заходит в здание — охранник его осматривает.",
          "Охранник может сразу сказать что-то (res), например: «Стоп, где документ?!»",
          "Если всё ок — охранник нажимает кнопку (next()), и клиент идёт дальше.",
        ],
        whatWeLearned: [
          "🧠 Middleware — это функция, которая срабатывает **до маршрута** и решает: что делать с клиентом.",
          "📦 Получает: req (сам клиент), res (ответ), и next() (разрешение пройти).",
          "🚫 Может остановить клиента и дать ему ответ сразу.",
          "✅ Может передать клиента дальше, если всё в порядке.",
        ],
        realExample: [
          "🌍 Пример из жизни приложения:",
          "Кто-то заходит на сайт — middleware проверяет токен или куки.",
          "Если нет токена — res говорит: «⛔ Вход запрещён!»",
          "Если всё есть — вызывается next(), и клиент попадает на маршрут.",
        ],
        whatNext: [
          "🔐 Теперь мы умеем ставить охрану, которая проверяет всех клиентов перед входом.",
          "🤝 Middleware можно делать глобальными (через <CodeHighlight>app.use()</CodeHighlight>), или **локальными — прямо внутри конкретного маршрута**.",
          "🚪 Например, можно сделать так, чтобы только маршрут `/vip` проверял, есть ли у клиента VIP-доступ.",
          "➡️ В следующем уроке мы научимся вставлять middleware прямо в маршруты — это как поставить охрану **только у одной двери**! 🚀",
        ],
      },

      {
        type: "code_files",
        title: "Охрана у одной двери — локальный middleware 🚪",
        content:
          "Мы уже умеем ставить охрану на входе в здание с помощью <CodeHighlight>app.use()</CodeHighlight>. Но что если охрана нужна **только у одной комнаты**?\n\n" +
          "Например, у нас есть VIP-комната, и туда могут заходить только клиенты с VIP-пропуском.\n\n" +
          "В Express можно вставить middleware прямо в маршрут — это и называется **локальный middleware**. Он будет срабатывать **только для этого пути**.",
        codeFiles: {
          "server.js": `// 🧾 Локальный middleware: проверка VIP-доступа
      const checkVip = (req, res, next) => {
        const isVip = req.headers['x-vip'] === 'true';
        if (!isVip) {
          return res.status(403).send('🚫 Доступ только для VIP-клиентов!');
        }
        next();
      };
      
      // 📍 VIP-маршрут с локальной охраной
      app.get('/vip', checkVip, (req, res) => {
        res.send('🎉 Добро пожаловать в VIP-комнату!');
      });

      // 📍 Обычный маршрут без охраны
      app.get('/public', (req, res) => {
        res.send('👋 Добро пожаловать в публичную зону!');
      });`,
          "client.js": `// 🚀 Клиент тестирует разные маршруты
import axios from 'axios';

// ✅ Успешный доступ к VIP-зоне
const accessVip = async () => {
  try {
    const response = await axios.get('http://localhost:3000/vip', {
      headers: {
        'x-vip': 'true'
      }
    });
    
    console.log('VIP доступ:', response.data);
    // Выведет: "🎉 Добро пожаловать в VIP-комнату!"
    
  } catch (error) {
    console.error('Ошибка VIP:', error.response?.data || error.message);
  }
};

// ❌ Попытка доступа к VIP без пропуска
const accessVipWithoutPass = async () => {
  try {
    const response = await axios.get('http://localhost:3000/vip');
    // Без заголовка x-vip
    ...
    console.log('Отказ в VIP:', error.response?.data);
    // Выведет: "🚫 Доступ только для VIP-клиентов!"
  }
};

// ✅ Доступ к публичной зоне (без проверок)
const accessPublic = async () => {
  try {
    const response = await axios.get('http://localhost:3000/public');
    
    console.log('Публичная зона:', response.data);
    // Выведет: "👋 Добро пожаловать в публичную зону!"
    
  } catch (error) {
    console.error('Ошибка публичной зоны:', error.response?.data || error.message);
  }
};

// 🚀 Тестируем все сценарии
accessVip();
accessVipWithoutPass();
accessPublic();`
        },
        metaphor: [
          "🚪 Метафора:",
          "Локальный middleware — это охранник, стоящий **только у одной двери**.",
          "Он проверяет только тех, кто пытается войти в VIP-комнату.",
          "В другие комнаты (маршруты) можно попасть без этой проверки.",
        ],
        whatWeLearned: [
          "🎯 Middleware можно вставлять прямо в маршрут как отдельную функцию.",
          "🧱 Такой middleware сработает **только для этого конкретного пути**.",
          "🔐 Это удобно для точечной защиты или проверок.",
          "🔄 Можно комбинировать сразу несколько middleware в маршруте.",
        ],
        realExample: [
          "🛂 Пример: ты заходишь на страницу '/admin', и только тогда срабатывает проверка, админ ты или нет.",
          "Другие страницы работают без этой проверки.",
        ],
        whatNext: [
          "💾 Мы научились ставить охрану только для определённых маршрутов.",
          "📩 Но бывает, что клиент не передаёт ничего в заголовках, а просто присылает **данные в теле запроса** (например, имя, email, пароль).",
        ],
      },

      {
        type: "intro",
        title: "Теперь ты — мастер middleware 🧠",
        content:
          "Ты только что освоил одну из самых **мощных концепций Express** — middleware.\n\n" +
          "Теперь ты знаешь, как ставить охрану:\n" +
          "— 💂‍♂️ На весь сервер (глобально через <CodeHighlight>app.use()</CodeHighlight>)\n" +
          "— 🚪 На конкретную дверь (локально в маршруте)\n\n" +
          "Это фундамент для авторизации, логирования, защиты, валидации и многого другого.\n\n" +
          "📦 И ты не ограничен одним middleware: можно ставить несколько подряд, вызывать `next()`, и даже подключать готовые из npm (например, `express.json()`, `cors`, `helmet`, `morgan`).\n\n" +
          "🔧 Middleware — это не просто охрана. Это мозг, фильтр и щит твоего сервера.",
        metaphor: [
          "🏨 Метафора:",
          "Ты построил **отель с умной охраной**.",
          "Где у каждой двери может стоять свой охранник.",
          "А на главном входе — дежурный, который ведёт журнал и проверяет всех.",
          "Ты управляешь потоком гостей, их безопасностью и даже тем, что они могут принести с собой.",
        ],
        whatWeLearned: [
          "🧠 Middleware — промежуточные функции, которые работают до маршрута.",
          "🔁 Бывают глобальные и локальные.",
          "📌 Используются для логирования, защиты, валидации, обработки тела и т.п.",
          "🧰 Можно использовать несколько подряд и комбинировать.",
        ],
        whatNext: [
          "📩 Иногда клиент не передаёт ничего в headers, а просто присылает данные в теле запроса (например, JSON).",
          "📦 А Express по умолчанию их не понимает — нужно помочь ему прочитать тело письма!",
          "➡️ В следующем уроке мы подключим <CodeHighlight>express.json()</CodeHighlight> — middleware, который читает тело запроса!",
        ],
      },
    ],
    []
      ...
      {showCongratulations && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          ...
          >
            ...
            <p className="text-gray-600 mb-6">Вы успешно завершили урок "Middleware и безопасность"!</p>
            <div className="bg-green-50 rounded-lg p-4 mb-6">
              <p className="text-green-800 font-medium mb-2">🎯 Вы изучили:</p>
              <ul className="text-green-700 text-sm space-y-1">
                <li>• Глобальные middleware (app.use)</li>
                <li>• Локальные middleware в маршрутах</li>
                <li>• Проверку заголовков и авторизацию</li>
                <li>• Отправку запросов с axios и middleware</li>
              </ul>
            </div>
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-2">
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium text-gray-600">📚 Урок 4: Middleware и безопасность</span>
