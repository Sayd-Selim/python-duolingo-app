...
function ExpressLesson6() {
  ...
  const steps = useMemo(
    () => [
      {
        type: "code_files",
        title: "Проверка анкет — валидация данных 📋✅",
        content:
          "Наш отель научился читать анкеты клиентов (JSON из тела запроса). Но теперь появилась новая задача:\n\n" +
          "**А что если клиент пришлёт пустое имя или номер в виде текста «abc»?**\n\n" +
          "📛 Такие анкеты нам не подходят! Нам нужно **проверять данные перед заселением**.\n\n" +
          "Это называется **валидация данных** — когда мы убеждаемся, что поля в теле запроса заполнены правильно.\n\n" +
          "В этом уроке мы научимся делать простую ручную проверку данных — без библиотек. Мы сами напишем «охранника», который проверит анкету перед заселением.",
        codeFiles: {
          "server.js": `// 🧾 Подключаем помощника для чтения JSON
  app.use(express.json());
  
  // 🛂 Обрабатываем POST-запрос с валидацией
  app.post('/checkin', (req, res) => {
    const { name, room } = req.body;
  
    // ❌ Проверка на отсутствие данных
    if (!name || !room) {
      return res.status(400).send('⛔ Имя и номер обязательны!');
    }
  
    // ❌ Проверка на тип данных
    if (typeof name !== 'string' || typeof room !== 'string') {
      return res.status(400).send('🚫 Имя и номер должны быть строками!');
    }
  
    // ❌ Дополнительные проверки (длина имени)
    if (name.trim().length < 2) {
      return res.status(400).send('✂️ Имя слишком короткое!');
    }
  
    // ✅ Всё хорошо — заселяем
    res.send(\`✅ Гость \${name} заселён в номер \${room}!\`);
  });`,
          "client.js": `// 🚀 Примеры разных анкет
  import axios from 'axios';
  
  // ✅ Успешная анкета
  axios.post('http://localhost:3000/checkin', {
    name: 'Адам',
    room: '305'
  }).then(res => {
  ...
  // ❌ Отправка с пустым именем
  axios.post('http://localhost:3000/checkin', {
    name: '',
    room: '101'
  }).catch(err => {
    console.log('Ошибка:', err.response?.data); // "⛔ Имя и номер обязательны!"
  });
  
  // ❌ Отправка с неправильным типом
  axios.post('http://localhost:3000/checkin', {
    name: 'Ибрагим',
    room: 123 // число вместо строки
  }).catch(err => {
    console.log('Ошибка:', err.response?.data); // "🚫 Имя и номер должны быть строками!"
  });`
        },
        metaphor: [
          "🧾 Метафора:",
          "Клиент приносит анкету — но вместо имени пишет «123», а номер комнаты не указывает вовсе.",
          "Наш охранник говорит: «⛔ Стоп! Сначала заполни анкету правильно!»",
          "Валидация — это фильтр, который не пускает ошибки внутрь сервера.",
        ],
        whatWeLearned: [
          "📋 Валидация — это проверка данных перед обработкой.",
          "❌ Если не проверять — можно получить пустые или некорректные значения.",
          "✅ Мы можем вручную проверять наличие полей, их тип и содержимое.",
          "🧱 Это первый шаг к надёжному серверу.",
        ],
        realExample: [
          "💡 Пример из жизни:",
          "Ты приходишь на ресепшн и сдаёшь пустую анкету. Ресепшионист скажет: «Заполните, пожалуйста!»",
          "Если в анкете номер комнаты — это «три собаки», тебя тоже не заселят.",
        ],
        whatNext: [
          "📋 Мы научились проверять данные вручную.",
          "🤔 Но когда анкета большая (имя, email, телефон, дата, и т.д.) — вручную становится неудобно.",
          "🧰 В следующем уроке мы подключим библиотеку <CodeHighlight>joi</CodeHighlight>, чтобы описывать и проверять данные по схеме!",
        ],
      },
      {
        type: "code_files",
        title: "Проверка анкеты с охранником — express-validator 🛂✅",
        content:
          "Мы уже поняли, что **клиенты могут отправлять невалидные данные**: пустые строки, числа вместо текста и т.д.\n\n" +
          "Вручную проверять каждое поле — неудобно. Нам нужен **специальный охранник**, который:\n\n" +
          "— знает правила анкеты\n" +
          "— сам проверит поля\n" +
          "— скажет, что не так\n\n" +
          "В Express есть такая библиотека — <CodeHighlight>express-validator</CodeHighlight>. Она позволяет прямо в маршрутах задавать правила валидации и проверять результат.",
        codeFiles: {
          "server.js": `// 📦 Устанавливаем express-validator: npm install express-validator
  ...
  // 🛂 Охранник проверяет анкету перед заселением
  app.post(
    '/checkin',
    [
      check('name')
        ...
        .isLength({ min: 2 }).withMessage('✂️ Имя слишком короткое'),
      check('room')
        .trim()
        .notEmpty().withMessage('⛔ Номер комнаты обязателен!')
        .isString().withMessage('🚫 Номер комнаты должен быть строкой'),
    ],
    (req, res) => {
  ...
  axios.post('http://localhost:3000/checkin', {
    ...
    console.log('Ошибка:', err.response?.data.errors?.[0]?.msg);
    // "⛔ Имя обязательно!"
  });
  
  // ❌ Номер комнаты не строка
  axios.post('http://localhost:3000/checkin', {
    name: 'Саид',
    room: 123
  }).catch(err => {
    console.log('Ошибка:', err.response?.data.errors?.[0]?.msg);
    // "🚫 Номер комнаты должен быть строкой"
  });`
        },
        metaphor: [
          "🛂 Метафора:",
          "Теперь у нас появился **охранник на входе**.",
          "Он получает анкету, проверяет каждое поле и говорит: «Это не по правилам!»",
          "`express-validator` — это как форма, где каждое поле проходит строгую проверку, и только после этого человек проходит дальше.",
        ],
        whatWeLearned: [
          "✅ `express-validator` позволяет проверять поля в маршрутах Express.",
          "🔍 Можно задавать условия: обязательность, длину, тип и т.д.",
          "🧾 Все ошибки автоматически собираются в массив.",
          "📦 Это удобно и масштабируемо — легко добавлять новые поля и правила.",
        ],
        realExample: [
          "💡 Пример из жизни:",
          "Ты приходишь в гостиницу, и тебе говорят: «Убедись, что имя не пустое, а номер — строка».",
          "Если что-то не так — анкету возвращают с конкретной ошибкой.",
        ],
        whatNext: [
          "🧱 Мы умеем валидировать анкеты на сервере.",
          "🔐 Но можно пойти дальше — создавать **схемы валидации отдельно**, чтобы переиспользовать их в разных маршрутах.",
          "➡️ В следующем уроке мы научимся **создавать валидаторы в отдельных файлах** и подключать их как middleware!",
        ],
      },
      {
        type: "code_files",
        title: "Переиспользуемые охранники — валидаторы в отдельных файлах 🔁🛡️",
        content:
          "Мы уже научились проверять поля прямо внутри маршрута.\n\nНо если анкет много: регистрация, заказ, оплата — **нельзя дублировать проверки везде**.\n\n🧱 Решение: **создать файл с валидаторами** и подключать их по мере необходимости. Это делает код:\n\n— чище\n— переиспользуемым\n— удобным для масштабирования.",
        codeFiles: {
          "middleware/validators.js": `// middleware/validators.js
      ...
      export const checkinValidator = [
        ...
        check('room')
          .trim()
          .notEmpty().withMessage('⛔ Номер обязателен!')
          .isString().withMessage('🚫 Номер должен быть строкой'),
      ];`,
          "server.js": `// server.js
      import express from 'express';
      import { validationResult } from 'express-validator';
      import { checkinValidator } from './middleware/validators.js';
      
      const app = express();
      app.use(express.json());
      
      app.post('/checkin', checkinValidator, (req, res) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
          return res.status(400).json({ errors: errors.array() });
        }
      ...
      axios.post('http://localhost:3000/checkin', {
        ...
        metaphor: [
          "📦 Метафора:",
          "Ты нанял 3 охранников — у каждого своя анкета: для заселения, для питания, для досуга.",
          "Ты не пишешь каждому охраннику, как проверять данные — ты даёшь им готовую инструкцию.",
          "Так и в Express — валидаторы становятся **шаблонами-проверками** для разных маршрутов.",
        ],
        whatWeLearned: [
          "🔁 Мы можем выносить правила валидации в отдельные файлы.",
          "📦 Это делает код читаемым, чистым и модульным.",
          "🧱 Один и тот же валидатор можно использовать в нескольких местах.",
        ],
        realExample: [
          "💡 Пример из жизни:",
          "Ты не пишешь каждый раз анкету заново, а просто используешь шаблон для каждого гостя.",
          "Если анкета одна — можно использовать её снова и снова.",
        ],
        whatNext: [
          "🔐 Мы можем валидировать данные прямо на входе.",
          "🛠️ В следующем уроке добавим **автоматическую сборку ошибок и форматированный ответ** — чтобы не писать `validationResult` в каждом маршруте!",
        ],
        motivation:
          "Чистый и переиспользуемый код — залог стабильного роста проекта. Когда валидация вынесена в модули, ты быстрее пишешь и реже ошибаешься.",
      },
      {
        type: "code_files",
        title: "Точное попадание — проверка тела запроса через body() 🎯📦",
        content:
          "Иногда мы хотим чётко указать: **проверяй только `body`**, не трогай `query` или `params`.\n\nВ этом нам помогает метод <CodeHighlight>body()</CodeHighlight> из `express-validator` — он явно говорит:\n\n🧾 «Проверь только данные, пришедшие в теле запроса».\n\nЭто делает код яснее, особенно в сложных приложениях, где много источников данных.",
        codeFiles: {
          "middleware/validators.js": `// middleware/validators.js
      ...
      axios.post('http://localhost:3000/checkin', {
        ...
        metaphor: [
          "🎯 Метафора:",
          "Ты говоришь охраннику: «Проверяй только то, что лежит в анкете. Не заглядывай в карманы и не спрашивай имя в коридоре.»",
          "`body()` — это строгая инструкция: смотри только в тело запроса!",
        ],
        whatWeLearned: [
          "📦 `body()` проверяет только данные из `req.body`.",
          "📛 Это полезно, если ты разделяешь валидацию `params`, `query` и `body`.",
          "📐 Код становится более читаемым и предсказуемым.",
        ],
        realExample: [
          "💡 Пример из жизни:",
          "Ты заполняешь анкету в бумажном виде. Сотрудник смотрит **только на бумагу**, а не расспрашивает дополнительно.",
          "Это помогает избежать путаницы и лишней информации.",
        ],
        whatNext: [
          "🧱 У нас уже есть модульные валидаторы.",
          "⚙️ Давай теперь сделаем универсальную middleware `validate()` — она будет автоматически обрабатывать ошибки и упрощать маршруты!",
        ],
        motivation:
          "`body()` делает валидацию более точной и предсказуемой. Это особенно важно в больших приложениях с разными маршрутами и источниками данных.",
      }
