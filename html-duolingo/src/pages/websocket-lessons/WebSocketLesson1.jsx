...
function WebSocketLesson1() {
  ...
  const steps = useMemo(
    () => [
      {
        type: "intro",
        title: "Как работает WebSocket? Создаём свой сервер 📡",
        content:
          "WebSocket — это технология, которая позволяет установить **постоянное соединение** между клиентом и сервером.\n\n" +
          "В отличие от <CodeHighlight>HTTP</CodeHighlight>, где каждое сообщение требует нового запроса, WebSocket открывает канал, по которому данные могут **свободно двигаться в обе стороны**.\n\n" +
          "Сегодня мы создадим свой WebSocket-сервер **на чистом Node.js**, без Socket.IO, чтобы разобраться, как всё устроено под капотом.\n\n" +
          "🎯 Цель: Реализовать минимальный WebSocket-сервер вручную и понять, как работает постоянное двустороннее соединение.",

        metaphor: [
          "📡 Метафора:",
          "Обычное HTTP-соединение — это как почтальон, который каждый раз приходит, забирает письмо и уходит.",
          "А <CodeHighlight>WebSocket</CodeHighlight> — это как телефонная линия: вы один раз набрали номер и можете говорить сколько угодно, пока соединение не оборвётся.",
          "Сегодня мы натянем такую линию связи между клиентом и сервером.",
        ],

        motivation:
          "Понимание WebSocket на низком уровне даст тебе суперсилу: ты сможешь создавать чаты, лайв-обновления, игры и всё, что требует моментальной обратной связи!",

      },
      {
        type: "code",
        title: "Создаём минимальный WebSocket-сервер на Node.js 🚀",
        content:
          "WebSocket-соединение начинается с обычного HTTP-запроса, но с заголовком <CodeHighlight>Upgrade: websocket</CodeHighlight>.\n\n" +
          "Мы можем «поймать» этот момент с помощью события <CodeHighlight>server.on('upgrade')</CodeHighlight> и вручную установить двустороннее соединение.\n\n" +
          "В этом примере мы создаём сервер, который принимает WebSocket-подключения и отвечает клиенту подтверждением соединения.",
        code: `// 📁 websocket-server.js
      ...
      server.on('upgrade', (req, socket) => {
        // Проверяем, что клиент действительно хочет установить WebSocket-соединение
        if (req.headers['upgrade'] !== 'websocket') {
          socket.end('HTTP/1.1 400 Bad Request');
          return;
        }
      
        // Получаем ключ из заголовков клиента
        const secWebSocketKey = req.headers['sec-websocket-key'];
      
        // Формируем ключ для ответа (по стандарту WebSocket)
        const hash = crypto
          .createHash('sha1')
          .update(secWebSocketKey + '258EAFA5-E914-47DA-95CA-C5AB0DC85B11')
          .digest('base64');
      
        // Отправляем заголовки, чтобы подтвердить апгрейд соединения
        ...
        socket.write('🎉 Добро пожаловать в WebSocket-сервер!');
      });
      ...
      server.listen(3000, () => {
        ...
        metaphor: [
          "🔌 Метафора:",
          "Подключение WebSocket — как секретный рукопожатие: клиент стучится, говорит, что хочет открыть постоянную линию.",
          "Если сервер понимает запрос — он выполняет обряд рукопожатия (handshake) и соединение становится постоянным.",
          "Теперь они связаны напрямую, как по телефону — без ожидания каждого ответа.",
        ],
      
        motivation:
          "Своими руками ты установил базовое WebSocket-соединение! Теперь ты понимаешь, как начинается «живое» общение между клиентом и сервером.",
      
        whatWeLearned: [
          "✅ Что мы узнали в этом шаге:",
          "- Как работает WebSocket Handshake.",
          "- Что такое <CodeHighlight>server.on('upgrade')</CodeHighlight>.",
          "- Как вручную подтверждать соединение по протоколу WebSocket.",
          "- Как передать простое сообщение после соединения.",
        ],
      
        whatNext: [
          "🧠 Следующий шаг: научимся принимать и отправлять сообщения по WebSocket-соединению — в обе стороны!",
        ]
      },
...
export default WebSocketLesson1;
